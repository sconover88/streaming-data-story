        ...
        <ul className="flex gap-4">
          {SECTION_LINKS.map((section) => (
            <li key={section.id}>
              <a
                href={`#${section.id}`}
                className="group text-gray-100 hover:text-white focus:text-white px-4 py-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 transition relative overflow-hidden"
                aria-label={`Jump to ${section.label} section`}
              >
                <span className="relative z-10">{section.label}</span>
                <span
                  className="absolute inset-0 z-0 opacity-0 group-hover:opacity-100 group-focus:opacity-100 transition-opacity duration-200 rounded"
                  style={{
                    background: '#a259ec',
                  }}
                  aria-hidden="true"

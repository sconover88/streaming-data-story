    ...
    <div className="flex flex-wrap gap-2" role="group" aria-label="Platform toggle">
      {platforms.map((platform) => (
        <button
          key={platform.id}
          type="button"
          className={`px-3 py-1 rounded font-medium border-2 focus:outline-none focus:ring-2 focus:ring-blue-500 transition text-sm ${selected.includes(platform.id) ? 'bg-white text-gray-900 border-white' : 'bg-gray-800 text-gray-200 border-gray-700'}`}
